...
A serological survey contains primarily the age and seropositivity status of the individuals, and can also contain information such as the sex, the location and the year of sampling, categories of individuals that ahave different risk of exposure ... 

Let's work on a toy example with simulated data: 500 individuals aged 1 to 70 yo, with a probability of infection of 0.2. 

```{r dataexample1, eval=TRUE, echo=FALSE, message = FALSE}
library(Rsero)
p.infection=0.2
N.samples=500
...
```{r dataexample2, eval=TRUE, echo=FALSE}
simulated.survey  = SeroData(age_at_sampling = age, Y = seropositive) 
```
That's the minimum requirement for creating a *SeroData* and it can be already used for the model fit. 
...
```{r dataexample3, eval=TRUE, echo=FALSE}
...
```{r dataexample5, eval=TRUE, echo=FALSE}
sex= c(rep('males',250), rep('females', 250))
...
```{r seroprevalenceplot, eval=TRUE}

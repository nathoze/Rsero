...
Stan is based on C++ and its necessary to install Rtools on Windows computers. Rtools version 4.0 and more recent can be found [here](https://cran.r-project.org/bin/windows/Rtools/)
 ...
 To install the package *Rsero*, use the devtool commande 
...
Second, we test the model of a constant force of infection and imperfect sensibility and specificity (model 9). In this model, the user has to provide the values of the sensibility and specificity, here se = 0.99 and sp = 0.95.
...
Third, we test the model of one outbreak and imperfect sensibility and specificity (model 10).  
```{r print_model_10, warning=FALSE, message = FALSE}
print(model[[10]])
```
```{r fit_3, warning=FALSE, message = FALSE}
...
C_1 = compute_information_criteria(Fit_1)
C_2 = compute_information_criteria(Fit_2)
C_3 = compute_information_criteria(Fit_3)

print(C_1)
print(C_2)
print(C_3)
```
The DIC of model 10 is significantly lower for model 10 than for model 6 and 9. This indicates that model 10 (outbreak model with imperfect sensitivity) is more adequate at fitting the data. ``
...
p = seroprevalence.fit(Fit_1)
print(p)
```

For the model 9,
```{r seroprevalence.plot2, warning=FALSE, message = FALSE}
p = seroprevalence.fit(Fit_2)
print(p)
```

And finally for model 10,  
```{r seroprevalence.plot3, warning=FALSE, message = FALSE}
p = seroprevalence.fit(Fit_3)
print(p)
```

According to this model, an outbreak occurred around 30 years before the survey. The non-zero seroprevalence in the younger individuals is explained here by false positives (implied by the sp parameter).
  
 

The posterior distribution can be plotted using the command 
```{r plot_posterior, warning=FALSE, message = FALSE}
plot_posterior(Fit_3)
```
...
traceplot_Rsero(Fit_3)
```

Additional diagnostics for the HMC and NUTS are available using standard *rstan* functions. The following functions are not intended for a precise diagnostic of individual parameters but rather to quickly identify if acceptable values are reached for all parameters. The Rhat compares the between and within-chain estimates and is a measure of good mixing should be lower than 1.1. The effective sample size is a measure of "how much independent information there is in autocorrelated chains" (Kruschke 2015) and it should be larger than 200 to provide a stable parameter estimate. This roughly indicates whether 
```{r Diagnostics, warning=FALSE, message = FALSE}
stan_diag(Fit_3$fit,info = c("sample")) # shows diagnostic (loglikelihood, acceptance rate,etc)
stan_rhat(Fit_3$fit) # show the Rhat statistic
stan_ess(Fit_3$fit) # show the ratio of effective sample size over total sample size
``` 


## Applying the model to Chikungunya data in the different regions
We apply model 10 the dataset "chik.sero" which contains information the survey participant sampling locations.

```{r fit_4_regional, warning=FALSE, message = FALSE}
Fit_4 = fit(data = chik.sero,
          model = model[[10]],
          chain=4,
          cores=4, 
          iter=5000)

...
```{r data1, echo=TRUE, warning=FALSE}
library(Rsero)
```
...
model <- FOImodel(type = "intervention", K = 2)
model <- FOImodel(type = "outbreak", K = 1)
model <- FOImodel(type = "constantoutbreak", K = 2)
model <- FOImodel(type = "independent")
print(model)
```
...
We write such models as `seroreversion = 1`. By default `seroreversion = 0`.
...
To call such background infection the line `background=1` must be added to the model definition. By default `background = 0`.
...
###  Background infection
  ...
  Cross-reactivity does not modify the formulas for the force of infection, but it changes the likelihood. With $p_B$ the probability that an individual is cross-reactive, the contribution to the likelihood of a seropositive individual is
...
Let's look at an example dataset provided with the package of a serological survey of CHIKV in Vietnam. This survey was described in Quan et al., 2018 (link) and includes 546 individuals sampled in 4 different areas of the country.  
...
```
We could have written data(chikv_vietnam) too
```{r vietnam2, echo=TRUE, warning=FALSE}
 data("chikv_vietnam")
```
We defined categories corresponding to the different regions. This can be seen in the column *category* of the dataset *chikv_vietnam* that associate to each individual one  of the four region: 
```{r vietnam3, echo=TRUE, warning=FALSE}
print(levels(chikv_vietnam$category))
...
We defined an *intervention* model, to describe that we expect a drop of the force of infection at a certain point in time. The parameter *cat_lamba = TRUE* is used to set different relative values of the force of infection while keeping equal the timing of the drop in infection probability. Therefore, the fit is done simultaneously on the four regions. 
-code fit
...
We implemented three different measures of model adequacy: The deviance information criterion (DIC), the Aikaike information criterion (AIC), the widely-applicable information criterion (WAIC). See Gelman Ref for a detailed explanation of all three measures.
...
Flat priors were chosen for all parameters except the force of infection in the *independent* model. 

### Default values of the hyperparameters

The default values of the hyperparameters are given for uniform distributions. The first and second parameters are respectively the lower and upper boundary of the distributions.  
...
model <- FOImodel(type = "outbreak", K = 2, priorRho1 = 0.009, priorRho2 = 0.011)
print(model)
```
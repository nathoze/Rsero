...
Different measures of model adequacy were implemented: The deviance information criterion (DIC), the Akaike information criterion (AIC), the  Watanabe-Akaike information criterion (WAIC). 
...
The the *independent* model, the prior distribution of each annual probability of infection is drawn with a uniform distribution $U(priorY1, priorY2)$ with default values
```{r independent}
priorY1 = 0
priorY2 = 10
```
...
An exponential distribution was implemented $\rho \sim  Exp(2)$
```{r priorsrho}
priorRho = 2
```
...
In the case of the `"outbreak"` and the `"constantoutbreak"` models each outbreak can be defined by a specific set of prior distributions. Here we set `K=2`. For the first outbreak, the prior for `T` is uniform(10,30), the prior for alpha is uniform(0,5). For the second outbreak, the prior for `T` is uniform(40,70), the prior for alpha is uniform(0,1).

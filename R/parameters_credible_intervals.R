  ...
  if(FOIfit$model$type %in% model.list('All models')){ 
    ...
    if(FOIfit$model$type  %in% model.list('Outbreak models')){
      ...
      for(i in 1:K){
        ...
        params <- add.quantiles.text(params,variable  = chainsout$beta[,i], name = paste('beta',i), quants= quants, quantilestext=quantilestext )
    ...
    if(FOIfit$model$type %in% model.list('I models')){
      ...
      for(i in 1:K){
        if (i>1){ 
          ...
          params <- add.quantiles.text(params,variable  = Years[,i], name = paste('Year_',i), quants= quants, quantilestext=quantilestext )
          
        }
        #        LL <- chains$logitlambda[,i]
        LL <- chains$foi[,i]
        params <- add.quantiles.text(params,variable  = LL, name = paste('FOI_',i), quants= quants, quantilestext=quantilestext )
    ...
    if(FOIfit$model$type=='constant'){
      ...
      L = chains$lambda
      params <- add.quantiles.text(params,variable  = L[,dim(L)[2]], name =paste('Constant'), quants= quants, quantilestext=quantilestext )
    ...
    if(FOIfit$model$type=='constantoutbreak'){
      ...
      L = chains$constant
      params <- add.quantiles.text(params,variable  = L, name =paste('Constant'), quants= quants, quantilestext=quantilestext )
    ...
    if(FOIfit$model$type=='independent'){
      ...
      for(k in seq(1,dim(L)[2])){
        params <- add.quantiles.text(params, variable=L[,k], name = paste0('Y_',FOIfit$data$sampling_year-k) , quants=quants, quantilestext=quantilestext)
